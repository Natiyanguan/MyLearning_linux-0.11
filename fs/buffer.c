...
// 由链接器 ld 在链接整个程序时设置的一个外部变量，帮我们计算好了整个内核代码的末尾地址，实现内核程序和缓冲区的划分
// 主内存区与缓冲区的分界线是定死的2M，缓冲区和内核程序的总计2M空间，分界线为end
extern int end;
...
void buffer_init(long buffer_end)
{
	...
	if (buffer_end == 1<<20)
		...
		b = (void *) buffer_end;
    // 这段代码用于初始化缓冲区，建立空闲缓冲区块 循环链表 ，并获取系统中缓冲块数目。
...
//一块空间，分别给了一对儿一对儿的缓冲头和缓冲块，缓冲头就是用来寻找缓冲块的。
// 缓冲头向上增长，而缓冲块向下增长
//								 buffer_end
//        					————  	1024 b缓冲块--------   b_data
//2M缓冲区					|		1024 b缓冲块-----  |
//    buffer_Memory	————————|		1024 b缓冲块--  |  |
//end						|		弃用块       |  |  |
//	  内核程序				|		h缓冲头-------  |  |
//0           				|		h缓冲头----------  |
//            				————	h缓冲头-------------
//								start_buffer
	...
	while ( (b -= BLOCK_SIZE) >= ((void *) (h+1)) ) {
		...
		// 完成缓冲区和缓冲块初始化后，创建以free_list为开头的双向链表
//							   |-------------
//	  						h缓冲头-------	|
//        					 |  |		|	|
//							h缓冲头		|	|		连接首尾缓冲头
...
//        free_list ————----h缓冲头-------	|
//        					   |-------------
//	缓冲头就是具体缓冲块的管理结构，而 free_list 开头的双向链表又是缓冲头的管理结构
//	从 free_list 开始遍历，就可以找到这里的所有内容了
	h--;                                    // 让h指向最后一个有效缓冲块头
